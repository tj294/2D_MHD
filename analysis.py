...
if args['-n']:
    ...
    L_cond_vol = np.trapz(L_cond, x=z, axis=1)
    L_conv_vol = np.trapz(L_conv, x=z, axis=1)
    Nu_inst = 1 + (L_conv_vol/L_cond_vol)
    print(L_cond.shape)
    print(z.shape)
    print(L_cond[hl:hu].shape)
    print(z[hl:hu].shape)
    L_cond_cz = np.trapz(L_cond[:, hl:hu], x=z[hl:hu], axis=1)
    L_conv_cz = np.trapz(L_conv[:, hl:hu], x=z[hl:hu], axis=1)
